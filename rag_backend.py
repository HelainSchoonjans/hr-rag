...
def rag_llm():
    llm=BedrockLLM(
        ...
        model_kwargs={'max_tokens_to_sample': 3000
                      ,'temperature': 0.1,
                      'top_p': 0.9}
    )
    return llm
#6b write a function wich searches the user prompt
def respond(index, question):
    llm=rag_llm()
    hr_rag_response=index.query(question=question, llm=llm)
    return hr_rag_response """